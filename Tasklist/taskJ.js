window.addEventListener('load', () => {
    ...
    const addTask = (task) => {
        ...
        if (list_el.children.length >= MAX_TASKS) {
        ...
        task_edit_el.addEventListener('click', () => {
            if (task_edit_el.innerText.toLowerCase() == "edit") {
                ...
                if (input.value !== '' && input.value.length < max_char) {
                    ...
                    alert(`Task length must be less than ${max_char} characters.`);
                }
            }
        });
